...
(function () {
  ...
  const MENU_FALLBACK = [
    { code:'DASHBOARD', name:'Dashboard ภาพรวม', icon:'ti-layout-dashboard', url:'dashboard.html', color:'c1', sort_order:10,
      description:'ยอดขายรวม เป้า Achievement กราฟรายวัน/สัปดาห์/เดือน และ AI Insights' },
    { code:'PRODUCTS', name:'สินค้า / Top SKU', icon:'ti-package', url:'products.html', color:'c2', sort_order:20,
      description:'จัดอันดับ SKU ขายดี Top 10/20/50 แยกแพลตฟอร์มและแบรนด์' },
    { code:'CREATORS', name:'ครีเอเตอร์ & Blacklist', icon:'ti-users-group', url:'creators.html', color:'c3', sort_order:30,
      description:'อันดับครีเอเตอร์ ยอดแยก VDO vs LIVE และรายชื่อ Cancel Rate เกินเกณฑ์' },
    { code:'IMPORT', name:'นำเข้าข้อมูล', icon:'ti-upload', url:'import.html', color:'c4', sort_order:40,
      description:'อัปโหลดไฟล์จาก JST / TikTok Shop / Lazada / Creator / Ads' },
    { code:'SETTINGS', name:'ตั้งค่า & สิทธิ์', icon:'ti-settings', url:'settings.html', color:'c5', sort_order:50,
      description:'เป้ายอดขาย SKU Master Blacklist ผู้ใช้งาน และการมองเห็นระบบ' }
  ];
  ...
  const state = {
    ...
    menu: []            // รายการหน้าที่ตำแหน่งนี้เข้าได้ (ชั้นเดียว)
  };

  function indexMenu(rows) {
    state.menu = (rows || []).slice().sort(function (x, y) {
      return (x.sort_order - y.sort_order) ||
             String(x.name).localeCompare(String(y.name), 'th');
    });
  }

  /** ลิงก์ของเมนู — โครงสร้างชั้นเดียว ทุกแถวมี url ตรง ๆ */
  function linkOf(row) {
    return (row && row.url) ? row.url : 'index.html';
  }
  ...
  async function guard(_legacyId, opts) {
    ...
    // หน้า Portal ไม่ต้องอยู่ในเมนู
    const OPEN_PAGES = ['index.html'];
