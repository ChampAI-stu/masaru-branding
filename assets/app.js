...
(function () {
  ...
  async function loadProfile(user) {
    ...
    try { indexMenu(await rpc('brand_my_menu')); } catch (e) { indexMenu([]); }
    return state;
  }
  ...
  function renderShell(systemCode) {
    ...
    html += '<nav class="nav">';
    html += '<a href="index.html" class="back"><span class="ic">←</span>หน้าหลักทุกระบบ</a>';
    ...
    if (kids.length) {
      ...
      kids.forEach(function (k) {
        ...
        html += '<a href="' + esc(linkOf(k)) + '" class="' + (active ? 'active' : '') + '">' +
                '<span class="ic">' + esc(k.icon || '•') + '</span>' + esc(k.name) + '</a>';
      });
    }
    ...
    if (others.length) {
      ...
      others.forEach(function (r) {
        html += '<a href="' + esc(linkOf(r)) + '"><span class="ic">' + esc(r.icon || '▦') + '</span>' +
                esc(r.name) + '</a>';
      });
    }
    html += '</nav>';
  ...
  /* ---------------- 9) CHART DEFAULTS ---------------- */
  const COLORS = {
    navy: '#1F3864', blue: '#2E75B6', gold: '#C9A84C', red: '#D0212B',
  ...
  function chartDefaults() {
    ...
    window.Chart.defaults.color = '#6B7891';
  ...
  /* ---------------- 10) EXPORT ---------------- */
  window.MB = {
    CFG: CFG, sb: sb, state: state, COLORS: COLORS,
